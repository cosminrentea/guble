...
All options can be supplied on the commandline or by a corresponding environment variable with the prefix `GOBBLER_`.
...
or
```
docker run -e GOBBLER_LOG=info cosminrentea/gobbler
```
...
```
Visit the [`gobbler-cli` documentation](https://github.com/cosminrentea/gobbler/tree/master/client/cli) for more details.
...
|--- |--- |--- |--- |--- |
|--env|GOBBLER_ENV|development &#124; integration &#124; preproduction &#124; production|development|Name of the environment on which the application is running. Used mainly for logging|
|--health-endpoint|GOBBLER_HEALTH_ENDPOINT|resource/path/to/healthendpoint|/admin/healthcheck|The health endpoint to be used by the HTTP server.Can be disabled by setting the value to ""|
|--http|GOBBLER_HTTP_LISTEN|format: [host]:port||The address to for the HTTP server to listen on|
|--kvs|GOBBLER_KVS|memory &#124; file &#124; postgres|file|The storage backend for the key-value store to use|
|--log|GOBBLER_LOG|panic &#124; fatal &#124; error &#124; warn &#124; info &#124; debug|error|The log level in which the process logs|
|--metrics-endpoint|GOBBLER_METRICS_ENDPOINT|resource/path/to/metricsendpoint|/admin/metrics|The metrics endpoint to be used by the HTTP server.Can be disabled by setting the value to ""|
|--ms|GOBBLER_MS|memory &#124; file|file|The message storage backend|
...
|CLI Option|Env Variable|Values|Default|Description|
|--- |--- |--- |--- |--- |
|--apns|GOBBLER_APNS|true &#124; false|false|Enable the APNS module in general as well as the connector to the development endpoint|
|--apns-production|GOBBLER_APNS_PRODUCTION|true &#124; false|false|Enables the connector to the apns production endpoint, requires the apns option to be set|
|--apns-cert-file|GOBBLER_APNS_CERT_FILE|path/to/cert/file||The APNS certificate file name, use this as an alternative to the certificate bytes option|
|--apns-cert-bytes|GOBBLER_APNS_CERT_BYTES|cert-bytes-as-hex-string||The APNS certificate bytes, use this as an alternative to the certificate file option|
|--apns-cert-password|GOBBLER_APNS_CERT_PASSWORD|password||The APNS certificate password|
|--apns-app-topic|GOBBLER_APNS_APP_TOPIC|topic||The APNS topic (as used by the mobile application)|
...
|CLI Option|Env Variable|Values|Default |Description|
|--- |--- |--- |--- |--- |
|sms|GOBBLER_SMS|true &#124; false|false |Enable the SMS gateway|
|sms_api_key|GOBBLER_SMS_API_KEY|api key||The Nexmo API Key for Sending sms|
...
* __Commandline Client__: https://github.com/cosminrentea/gobbler/tree/master/client/cli
